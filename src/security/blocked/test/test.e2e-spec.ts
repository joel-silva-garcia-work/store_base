...
export enum INVALID_DTO {
  NULL_ACCESS_TYPE = 'NULL_ACCESS_TYPE',
  NO_ACCESS_TYPE = 'NO_ACCESS_TYPE',
  EMPTY_ACCESS_TYPE = 'EMPTY_ACCESS_TYPE',
  WRONG_ACCESS_TYPE_TYPE = 'WRONG_ACCESS_TYPE_TYPE',
  NO_TOKEN = 'NO_TOKEN',
  NO_ID = 'NO_ID',
}

const generateDto = () => {
    name: `Blocked ${randomString(10)}`,
    description: `Desc ${randomString(12)}`,
    isActive: true,
    accessType: randomString(12),
  }
  ...
  switch (invalid) {
    ...
    case InvalidStandardDTO.WRONG_DESCRIPTION_TYPE:
      ...
      break
        ...
        case INVALID_DTO.NULL_ACCESS_TYPE:
          invalidDto.accessType = null as any;
          break;
        case INVALID_DTO.NO_ACCESS_TYPE:
          delete invalidDto.accessType;
          break;
        case INVALID_DTO.EMPTY_ACCESS_TYPE:
          invalidDto.accessType = "" as any;
          break;
        case INVALID_DTO.WRONG_ACCESS_TYPE_TYPE:
          invalidDto.accessType = 123 as any;
          break;
...
describe('API Tests', () => {
  ...
  describe('Failed Cases', () => {
    describe('Failed Created Cases', () => {
      describe('Edge Cases', () => {
        test('should handle very long accessType', async () => {
                const createUrl = url + '/adicionar'
                const dto: any = generateDto()
                dto.accessType = 'a'.repeat(1000)
                const result = await createData(createUrl, dto, token)
                expect(result).toBeInstanceOf(HttpError)
