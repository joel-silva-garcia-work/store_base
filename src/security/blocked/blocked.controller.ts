...
@ApiTags('blocked')
@Controller('security/blocked')
export class BlockedController extends BaseControllerCRUD<
CreateBlockedDto,
UpdateBlockedDto,
BlockedService
> {
  ...
  @UseGuards(JwtGuard)
  @Post('adicionar')
  @ApiOperation({ summary: 'Crear un nuevo item en blocked' })
  ...
  async Add(
    @Body(new ValidationPipe({ transform: true })) createDto: CreateBlockedDto,
    @Req() request: Request
  ) {
  ...
  @UseGuards(JwtGuard)
  @Patch('actualizar')
  @ApiOperation({ summary: 'Actualizar un item existente en blocked' })
  ...
  async Edit(
    @Body(new ValidationPipe({ transform: true })) updateDto: UpdateBlockedDto,
    @Req() request: Request
  ) {
    const clientIp = request.socket.remoteAddress;
    const ipv4 = clientIp?.replace('::ffff:', '');
  ...
  @UseGuards(JwtGuard)
  @Put('cambiar-estado')
  @ApiOperation({ summary: 'Activar/Desactivar un item de blocked' })
  ...
  // @UseGuards(JwtGuard)
  // @Delete('eliminar')
  // @ApiOperation({ summary: 'Eliminar un item de blocked' })
