...
export class CategoriaController extends BaseControllerCRUD<
CreateCategoriaDto,
UpdateCategoriaDto,
CategoriaService
> {
  ...
  @UseGuards(JwtGuard)
  @Post('adicionar')
  @ApiOperation({ summary: 'Crear un nuevo item en categoria' })
  ...
  async Add(
    @Body(new ValidationPipe({ transform: true })) createDto: CreateCategoriaDto,
    @Req() request: Request
  ) {
  ...
  @UseGuards(JwtGuard)
  @Patch('actualizar')
  @ApiOperation({ summary: 'Actualizar un item existente en categoria' })
  ...
  async Edit(
    @Body(new ValidationPipe({ transform: true })) updateDto: UpdateCategoriaDto,
    @Req() request: Request
  ) {
    const clientIp = request.socket.remoteAddress;
    const ipv4 = clientIp?.replace('::ffff:', '');
  ...
  @UseGuards(JwtGuard)
  @Put('cambiar-estado')
  @ApiOperation({ summary: 'Activar/Desactivar un item de categoria' })
  ...
  // @UseGuards(JwtGuard)
  // @Delete('eliminar')
  // @ApiOperation({ summary: 'Eliminar un item de categoria' })
