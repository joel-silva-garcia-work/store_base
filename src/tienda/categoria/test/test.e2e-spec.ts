...
export enum INVALID_DTO {
  ...
  WRONG_ICON_TYPE = 'WRONG_ICON_TYPE',
  NO_TOKEN = 'NO_TOKEN',
  NO_ID = 'NO_ID',
}

const generateDto = () => {
    name: `Categoria ${randomString(10)}`,
    description: `Desc ${randomString(12)}`,
    isActive: true,
    icon: randomString(12),
  }
  ...
  switch (invalid) {
    ...
    case InvalidStandardDTO.WRONG_DESCRIPTION_TYPE:
      ...
      break
        ...
        case INVALID_DTO.NULL_ICON:
          invalidDto.icon = null as any;
          break;
        case INVALID_DTO.NO_ICON:
          delete invalidDto.icon;
          break;
        case INVALID_DTO.EMPTY_ICON:
          invalidDto.icon = "" as any;
          break;
        case INVALID_DTO.WRONG_ICON_TYPE:
          invalidDto.icon = 123 as any;
          break;
...
describe('API Tests', () => {
  ...
  describe('Failed Cases', () => {
    describe('Failed Created Cases', () => {
      describe('Edge Cases', () => {
        test('should handle very long icon', async () => {
                const createUrl = url + '/adicionar'
                const dto: any = generateDto()
                dto.icon = 'a'.repeat(1000)
                const result = await createData(createUrl, dto, token)
                expect(result).toBeInstanceOf(HttpError)
